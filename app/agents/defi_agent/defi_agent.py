...
def fetch_defi_pools(chain: str, symbol: str, min_tvl_usd: float, include_rewards: bool) -> List[Dict[str, Any]]:
    ...
    for p in data:
        ...
        reward_apr = float(p.get("apyReward", 0.0) or 0.0) / 100.0 if include_rewards else 0.0
...
def extract_params_from_prompt(prompt: str) -> Dict[str, Any]:
    t = _norm(prompt)
    risk = "high" if "high" in t else "medium" if ("medium" in t or "mid" in t) else "low"
    days = DEFAULTS["days"]
    ...
    if m_days:
        ...
        if m_num:
            try: days = int(m_num.group(1))
            except: pass
    chain = next((canon for alias, canon in CHAIN_ALIASES.items() if re.search(rf"\b{re.escape(alias)}\b", t)), None) or DEFAULTS["chain"]
    symbol = next((canon for tok, canon in TOKEN_ALIASES.items() if re.search(rf"\b{re.escape(tok)}\b", t)), None) or DEFAULTS["symbol"]
...
def resolve_and_fetch(user_prompt: str) -> Dict[str, Any]:
    ...
    pools = fetch_defi_pools(params["chain"], params["symbol"], params["min_tvl_usd"], params["include_rewards"])
    ...
    if a_markets and b_markets:
        a_ap = a_markets[0]["supply_apy"]; b_ap = b_markets[0]["supply_apy"]
        if a_ap > b_ap:
            verdict = f"{a_markets[0]['platform']} edges out for lenders"; better = a_markets[0]["platform"]
        elif b_ap > a_ap:
            verdict = f"{b_markets[0]['platform']} edges out for lenders"; better = b_markets[0]["platform"]
        else:
            verdict = "Both yield similar returns"; better = None
